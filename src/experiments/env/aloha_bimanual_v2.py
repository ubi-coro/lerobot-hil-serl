...
class AlohaBimanualEnvConfigV2(RobotEnvConfig):
    ...
    def initial_features(self):
        ft = super().initial_features
        action_dim = ft[PipelineFeatureType.ACTION][ACTION].shape[0]
        action_dim += 2 * len(self.robot)  # shadows
        ft[PipelineFeatureType.ACTION][ACTION].shape = (action_dim, )
        return ft

    def make_action_processor(self, teleoperators, device) -> DataProcessorPipeline:
        action_processor = super().make_action_processor(teleoperators, device)

        # swap intervention wrapper
