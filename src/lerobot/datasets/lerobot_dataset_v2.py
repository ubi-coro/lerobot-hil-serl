...
from lerobot.datasets.utils import get_safe_version, load_info, check_version_compatibility, load_stats, \
    cast_stats_to_numpy
from lerobot.datasets.v30.convert_dataset_v21_to_v30 import load_jsonlines
...
def load_episodes_v2(local_dir: Path) -> dict:
    episodes = load_jsonlines(local_dir / EPISODES_PATH)
    return {item["episode_index"]: item for item in sorted(episodes, key=lambda x: x["episode_index"])}

def load_tasks_v2(local_dir: Path) -> tuple[dict, dict]:
    tasks = load_jsonlines(local_dir / TASKS_PATH)
    tasks = {item["task_index"]: item["task"] for item in sorted(tasks, key=lambda x: x["task_index"])}
    task_to_task_index = {task: task_index for task_index, task in tasks.items()}
    return tasks, task_to_task_index

def is_valid_version_v2(version: str) -> bool:
...
class LeRobotDatasetV2Metadata:
    def __init__(
        self,
        repo_id: str,
        ...
        try:
            ...
            if is_valid_version_v2(self.revision):
    ...
    def load_metadata(self):
        ...
        self.tasks, self.task_to_task_index = load_tasks_v2(self.root)
