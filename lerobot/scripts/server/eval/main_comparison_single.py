...
CURVES : Dict[str, Tuple[Callable[[Path], bool], int, Dict]] = {
    ...
    "PPO"   : (
        lambda p: "ppo_sparse"  in str(p),
        WINDOW,
        {"lw":2,"color": "purple"},
    ),
    "SAC"  : (
        lambda p: "rlpd_sparse" in str(p) and "_itv_" not in str(p) and "_demos_" not in str(p) and "init_small" not in str(p),
        WINDOW,
        {"lw":2,"color": "orange"}
    ),
    "no-vision"  : (
        lambda p: "rlpd_sparse" in str(p) and "_no_vision_" in str(p),
        WINDOW,
        {"lw":2,"ls":"--","color":"#009E73"}
    ),
    "no-priors"  : (
        lambda p: "rlpd_dense" in str(p) and "_no_priors_" in str(p),
        WINDOW,
        {"lw":2,"ls":":","color":"#009E73"}
    ),
}
...
def main():
    ...
    fig, axes = plt.subplots(len(METRIC_TAGS), 1, figsize=(3.54, 2.4), sharex=True,
    ...
    for ax, metric_tag in zip(axes, METRIC_TAGS.keys()):
        ...
        if metric_tag == "cycle":
            ...
            axes[-1].plot([], [], label="Human (baseline)", **HUMAN_STYLE)
    ...
    plt.savefig("results/ablations.pdf", dpi=600, bbox_inches='tight', pad_inches=0, transparent=False)
    plt.savefig("results/ablations.png", dpi=400)
