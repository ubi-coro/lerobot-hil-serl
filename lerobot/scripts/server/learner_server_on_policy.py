...
def add_actor_information_and_train(
    ...
    while True:
        ...
        rollout = process_transitions(
            transition_queue=transition_queue,
            device=device,
            shutdown_event=shutdown_event,
        )

        ppo_batch = build_ppo_batch(rollout, policy, gamma, lam)
        ...
        for epoch in range(cfg.policy.epochs):
            for batch in iterate_minibatches(ppo_batch, cfg.batchsize):
                ...
                critic_grad_norm = torch.nn.utils.clip_grad_norm_(
                    parameters=policy.critic.parameters(), max_norm=clip_grad_norm_value
                ).item()
...
def iterate_minibatches(batch: Dict[str, Any], mb_size: int):
    n = batch["action"].shape[0]
    idxs = torch.randperm(n)
    for start in range(0, n, mb_size):
        mb_idxs = idxs[start : start + mb_size]
        yield {k: (v[mb_idxs] if torch.is_tensor(v) else {kk: vv[mb_idxs] for kk, vv in v.items()}) for k, v in batch.items()}
...
def process_transitions(
    ...
    rollout: list[dict] = []
    while len(rollout) < roll_len and not shutdown_event.is_set():
        ...
        for transition in transition_list:
            ...
            if transition.get("complementary_info", {}).get("is_intervention"):
                logging.warning("[LEARNER] Intervention detected in transition, skipping")
                continue

            rollout.append(transition)


def build_ppo_batch(rollout: List[Dict[str, Any]], policy: PPOPolicy, gamma: float, lam: float) -> Dict[str, torch.Tensor]:
    """Convert list[transition] into a dict of tensors + compute GAE & returns."""
    # Stack fields
    obs = {k: torch.stack([t["state"][k] for t in rollout]) for k in rollout[0]["state"]}
    actions = torch.stack([t["action"] for t in rollout])
    rewards = torch.stack([t["reward"] for t in rollout])
    dones = torch.stack([t["done"] for t in rollout])
    old_log_prob = torch.stack([t["log_prob"] for t in rollout])  # assumed in transition

    with torch.no_grad():
        # Values for all states incl. last next_state
        values = policy.critic(policy.actor.encoder(obs, cache=None)).squeeze(-1)
        # bootstrap value for final state (the last transition's next_state)
        last_next = {k: rollout[-1]["next_state"][k].unsqueeze(0) for k in rollout[-1]["next_state"]}
        last_val = policy.critic(policy.actor.encoder(last_next, cache=None)).squeeze(-1)
    values = torch.cat([values, last_val])  # len == T+1
