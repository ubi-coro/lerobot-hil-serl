...
class PushCubeEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    def __init__(self, *args, robot_uids: Literal["panda", "ur"] = "panda", robot_init_qpos_noise: float = 0.02, sparse_reward: bool = True, **kwargs):
    ...
    def _default_sensor_configs(self):
        ...
        pose_1 = sapien_utils.look_at([0.6, 0.7, 0.6], [0.0, 0.0, 0.35])
        pose_2 = sapien_utils.look_at([0.6, -0.7, 0.6], [0.0, 0.0, 0.35])
        return [
            CameraConfig(
                "left_image",
                pose=pose_1,
    ...
    def compute_normalized_dense_reward(self, obs: Any, action: Array, info: Dict):
        ...
        max_reward = torch.tensor([3.0], device=self.device)
        if self.sparse_reward:
            if info["success"]:
                info["success"] = torch.tensor([1.0], device=self.device)
                return torch.tensor([max_reward], device=self.device)
            else:
